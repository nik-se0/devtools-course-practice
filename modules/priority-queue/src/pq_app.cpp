...
void AppPQ::Help(const char* appname, const char* mes) {
    ...
    stream <<
        ...
        << "\n\t'get' - \tRetrieves the element on the top and returns its value out of queue "
...
std::string AppPQ::operator()(int argc, const char** argv) {
   ...
   if (argc == 2) {
       try {
           ...
           case 3:
               tmp = Q1.size();
               message += "\tA queue size = " + std::to_string(tmp);
               message += "\n";
               break;
           case 2:
               if (Q1.empty()) {
               ...
               break;
           case 4:
                message+= "\tAn insertion was made into the queue\n";
               Q1.pop();
               break;
           case 5:
               ...
               break;
           case 6:
                message+= "\tThe withdrawal was made from the queue a value of ";
               tmp = Q1.get();
               break;
           default:       throw std::string("Wrong number format!");
           }
       }
